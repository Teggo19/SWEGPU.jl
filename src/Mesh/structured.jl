...
function make_structured_mesh(n_x, n_y, spaceType, indType; alternative=false)
    #p = Progress(Int64((n_x+1)*(n_y+1)+ n_x*n_y))
    #mesh = Array{Float64, 3}(undef, n_x+1, n_y+1, 3)
    if alternative
        filename = "structs/structured_alt_$(n_x)x$(n_y)_$(spaceType)_$(indType)"
    else
...
using Random
function make_shuffled_mesh(n_x, n_y, spaceType, indType)
    #p = Progress(Int64((n_x+1)*(n_y+1)+ n_x*n_y))
    #mesh = Array{Float64, 3}(undef, n_x+1, n_y+1, 3)
    filename = "structs/structured_$(n_x)x$(n_y)_$(spaceType)_$(indType)_shuffled"
    if isfile("tmp/$filename.jld")
        println("Loading mesh from tmp/$filename.jld")
        return load_structures(filename)
    end
